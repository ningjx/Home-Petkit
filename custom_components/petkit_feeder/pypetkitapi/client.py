...
from .command import ACTIONS_MAP
from .const import (
    CLIENT_NFO,
    DEFAULT_COUNTRY,
    DEFAULT_TZ,
...
from .feeder_container import Feeder, FeederRecord, SoundList
from .litter_container import Litter, LitterRecord, LitterStats, PetOutGraph
from .purifier_container import Purifier
from .utils import get_timezone_offset
from .water_fountain_container import WaterFountain, WaterFountainRecord
...
class PetKitClient:
    ...
    async def _fetch_device_data(
        ...
        # 调试日志：打印 FeederRecord 的原始响应
        if data_class.__name__ == "FeederRecord":
            _LOGGER.debug(
                "FeederRecord API 响应 (endpoint=%s, params=%s): %s",
                endpoint,
                query_param,
                response,
            )
        ...
        if isinstance(response, list):
            if data_class.__name__ == "FeederRecord":
              _LOGGER.debug( "response 检测到列表" )
            device_data = [data_class(**item) for item in response]
            if data_class.__name__ == "FeederRecord":
              _LOGGER.debug( "device_data 值: %s", device_data )
        elif isinstance(response, dict):
            if data_class.__name__ == "FeederRecord":
              _LOGGER.debug( "response 没有检测到列表" )
